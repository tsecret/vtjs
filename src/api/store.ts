...
export class StoreAPI extends BaseAPI {
	async getStore(puuid: string): Promise<StorefrontResponse> {
		return this.fetch(
			...
			{ body: JSON.stringify({}), method: "POST", ttl: 5 * 60 * 1000 },
		);
	}
	...
	async getSkinByLevelId(levelId: string): Promise<SkinResponse> {
		return this.fetch(
			...
			`/v1/weapons/skinlevels/${levelId}`,
			{ headers: {} },
		);
	}

	async getBundleById(bundleId: string): Promise<SkinResponse> {
		return this.fetch("https://valorant-api.com", `/v1/bundles/${bundleId}`, {
			headers: {},
		});
	}

	async getSprayById(sprayId: string): Promise<SkinResponse> {
		return this.fetch("https://valorant-api.com", `/v1/sprays/${sprayId}`, {
			headers: {},
		});
	}
