...
export const StorePage = () => {
		...
		const init = async () => {
			...
			const accessoriesData = await Promise.all(
				accessories.map((item) =>
					item.type === "spray"
						? storeapi.getSprayById(item.uuid)
						: item.type === "buddy"
							? storeapi.getBuddieById(item.uuid)
							: item.type === "playercard"
								? storeapi.getPlayerCardById(item.uuid)
								: null,
				),
			);
	...
	return (
		<div className="flex flex-col p-8 space-y-4 max-w-6xl m-auto">
					...
					{bundles?.map((bundle, i) => (
						<div key={bundle.uuid} id={`bundle${i}`} className="carousel-item relative w-full">
							...
							<div className="absolute right-5 bottom-5">
								{bundlesResetTime && (
									<p>
										Ends in <Countdown date={bundlesResetTime[i]} />
									</p>
								)}
							</div>
							...
							<div className="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
								{bundles.length > 1 && (
									<a href={`#bundle${i - 1}`} className="btn btn-circle">
										❮
									</a>
								)}
								{bundles.length > 1 && (
			...
			<section id="skins" className="w-full">
				<div className="flex flex-row items-center justify-between">
					...
					{skinsResetTime && (
						<p>
							Resets in <Countdown date={skinsResetTime} />
						</p>
					)}
				</div>
			...
			<section id="accessories" className="w-full">
				<div className="flex flex-row items-center justify-between">
					<h2>Accessories</h2>
					{accessoriesResetTime && (
						<p>
							Resets in <Countdown date={accessoriesResetTime} />
						</p>
					)}
				</div>
				<div className="flex flex-row items-center justify-center space-x-4 m-auto px-8 my-8">
					{accessories?.map((skin) => (
						<div
							key={skin.uuid}
							className="flex flex-1 flex-col border-2 border-primary rounded-md p-4 min-h-48 max-w-64 justify-between tooltip"
						>
