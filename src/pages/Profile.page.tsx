	...
	return (
		<div className="flex flex-col items-center p-4">
			...
			<div className="flex flex-row space-x-4">
				<div className="card bg-base-200 shadow-xl p-6 rounded-2xl space-y-2">
					<div>
						<span className="text-2xl font-bold">{playerCard?.name}</span>
						<span> #{playerCard?.tag}</span>
					</div>
					...
					<div className="stats shadow">
						...
						<div className="stat">
							...
							<div className="stat-value">
								...
								{table.filter((match) => match.result === "loss").length}
							</div>
						</div>
			...
			{chartData?.length ? (
				<>
                    ...
                    className={clsx(
                      ...
                      "text-center",
                      refMatchId === match.matchId && "outline-2 outline-primary",
										)}
									>
										...
										<td>
											<div className="tooltip flex flex-row items-center" data-tip={match.rankBefore?.rankName}>
												...
												{match.rankBefore &&
													match.rankAfter &&
													(match.rankAfter?.tier > match.rankBefore?.tier ? (
														<ChevronsUp size={16} />
													) : match.rankAfter?.tier < match.rankBefore?.tier ? (
								...
								<table className="table table-xs text-center">
									...
									<tbody>
										{bestAgents?.map((agent) => (
											<tr
												key={agent.agentId}
												className={clsx(refAgentId === agent.agentId && "outline-2 outline-primary")}
											>
						...
						<div className="flex flex-col space-y-4">
							...
							<div className="overflow-x-auto">
								<table className="table table-xs text-center">
									...
									<tbody>
										{bestMaps?.map((map) => (
											<tr key={map.mapId} className={clsx(refMapId === map.mapId && "outline-2 outline-primary")}>
												<th className="flex flex-row items-center space-x-2">
