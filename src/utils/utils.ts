...
import type {
	...
	Rank,
	Streak,
} from "@/interface/utils.interface";
import type {
	Agent,
	AgentStats,
...
export const extractPlayerName = (
	puuid: string,
...
export const calculateMostPlayedServer = (matches: MatchDetailsResponse[]): MostPlayedServer => {
	...
	for (const match of matches) {
		...
		if (!(serverName in servers)) servers[serverName] = 0;
...
export const extractEncounters = (
	puuid: string,
	matches: MatchDetailsResponse[],
...
export const sortPlayersForProcessing = (
	players: PlayerNamesReponse[],
